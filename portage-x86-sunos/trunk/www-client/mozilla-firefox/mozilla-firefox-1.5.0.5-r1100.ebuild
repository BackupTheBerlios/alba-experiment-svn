...
KEYWORDS="-* amd64 ~arm hppa ~ia64 ~mips ppc sparc x86 ~x86-fbsd -x86-sunos"
SLOT="0"
LICENSE="MPL-1.1 NPL-1.1"
IUSE="java mozdevelop vanilla"
...
src_unpack() {
	...
	if ! use vanilla; then
		...
		if use x86-sunos; then
			# Fix a couple of "mysterious" compilation and linking issue on Solaris
			# -- brx, 2006-08-11
			(cd ${S} && epatch ${FILESDIR}/firefox-solaris-nsBidi.patch)
			(cd ${S} && epatch ${FILESDIR}/firefox-solaris-TestGtkEmbed-Makefile.in.diff)
		fi
