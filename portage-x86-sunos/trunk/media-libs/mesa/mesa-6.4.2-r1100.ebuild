...
src_unpack() {
	...
	echo "# Added by Gentoo ebuild" >> ${HOSTCONF}

	# Set default dri drivers directory
	use dri && echo "DEFINES += -DDEFAULT_DRIVER_DIR='\"/usr/$(get_libdir)/xorg/modules/dri\"'" >> ${HOSTCONF}
	...
	use dri && echo "DRI_DIRS = ${DRI_DRIVERS}" >> ${HOSTCONF}
	...
	use dri && echo "SRC_DIRS = glx/x11 mesa glu glw" >> ${HOSTCONF}
	# Remove glx/x11 for remove libdrm dependency, suitable for Solaris
	! use dri && echo "SRC_DIRS = mesa glu glw" >> ${HOSTCONF}
	...
	[[ ${USERLAND} != "SunOS" ]] && echo "PROGRAM_DIRS =" >> ${HOSTCONF}
