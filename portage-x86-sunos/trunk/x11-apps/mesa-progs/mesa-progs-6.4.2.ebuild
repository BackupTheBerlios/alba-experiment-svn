...
src_unpack() {
	...
	if use x86-sunos; then
		einfo "Removing -static flag for x86-sunos"
		sed -i -e "s:MKLIB_OPTIONS = -static:MKLIB_OPTIONS = :" ${HOSTCONF}
		einfo "Removing use of openwin libraries"
		sed -i 	-e "s:-L/usr/openwin/lib::g" \
				-e "s:-R/usr/openwin/lib::g" \
				-e "s:-I/usr/openwin/include::g" ${HOSTCONF}
		einfo "Adding -lX11 dependency"
		sed -i -e '/APP_LIB_DEPS/ { s/$/ $(GL_LIB_DEPS)/}' ${HOSTCONF}
	fi
}
