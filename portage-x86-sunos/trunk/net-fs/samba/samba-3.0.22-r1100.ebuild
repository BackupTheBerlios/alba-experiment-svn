...
src_unpack() {
	...
	epatch ${FILESDIR}/samba-python-libs.path
}
...
src_compile() {
	...
	use sun-ld && myconf="${myconf} ac_cv_prog_gnu_ld=no"
...
src_install() {
	...
	if use x86-sunos; then
		# Solaris has a command called profiles, so we change it
		mv ${D}/usr/bin/profiles ${D}/usr/bin/smbprofiles
	fi
