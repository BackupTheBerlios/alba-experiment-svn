...
src_unpack() {
	...
	use x86-sunos && epatch ${FILESDIR}/sunos-glx-makefile.patch
	use x86-sunos && epatch ${FILESDIR}/sunos-visibility.patch
	...
	if use x86-sunos; then
		einfo "sed for x86-sunos, do shared libs"
		sed -i -e "s/MKLIB_OPTIONS =.*/MKLIB_OPTIONS =/" ${HOSTCONF} || die "Cannot update ${HOSTCONF}"
	fi
...
fix_opengl_symlinks() {
	...
	if [[ ${CHOST} == *-freebsd* ]] || use x86-sunos; then
