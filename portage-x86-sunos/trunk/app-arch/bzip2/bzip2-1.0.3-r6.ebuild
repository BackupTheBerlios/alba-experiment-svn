...
src_unpack() {
	...
	if use sun-ld ; then
		einfo "Fixing internal name"
		sed -i 's/-Wl,-soname/-Wl,-h/' \
			"${S}"/Makefile-libbz2_so || die "Cannot sed."
	fi
...
src_install() {
	if ! use build ; then
		...
		! use sun-ld && gen_usr_ldscript libbz2.so

		if use x86-sunos && use sun-ld ; then
			dosym ../$(get_libdir)/libbz2.so.${PV} /usr/$(get_libdir)/libbz2.so
		fi
